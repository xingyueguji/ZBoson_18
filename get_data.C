...
void get_data(){
	...
	for (int i= 0; i<nentries; i++){
		...
		//Cut setup
		if (!(data->evtSel[1])) continue;
		if (!(data->evtSel[2])) continue;
		if (!(data->evtSel[3])) continue;
		...
		for (int j=0;j<data->candSize; j++){
			...
			if(abs(data->y[j]) > 2.4)continue;
			if (!(data->tightMuon1Cut(j,"POG") && data->tightMuon2Cut(j,"POG"))) continue;
	...
	for (int i= 0; i < data_same_sign->t1->GetEntries(); i++){
		...
		//Cut setup
		if (!(data_same_sign->evtSel[1])) continue;
		if (!(data_same_sign->evtSel[2])) continue;
		if (!(data_same_sign->evtSel[3])) continue;
		if (abs(data_same_sign->bestvtxZ) > 15) continue;

		int hiBin = 0;
		hiBin = data_same_sign->getcentrality(0); // FIXME: doZDC? hiBinVar?

		if (!(data_same_sign->trigHLT[6])) continue;
		...
		for (int j=0;j<data_same_sign->candSize; j++){
			...
			if(abs(data_same_sign->y[j]) > 2.4)continue;
			if (!(data_same_sign->tightMuon1Cut(j,"POG") && data_same_sign->tightMuon2Cut(j,"POG"))) continue;
			...
			bool passesAco[3] = {1,1,1};
			if (data_same_sign->pT[j] < 1.25 && acoplanarity < 0.001) passesAco[0] = false;
