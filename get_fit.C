...
void get_fit(int opt = 5, bool use_unbin = 1, bool use_w2_unbin = 0){
	//need 11 objs
	// opt 1 == raw
	// opt 2 == y
	// opt 3 == eff
	...
	// opt 5 == eta
	// opt 6 == eta+eff
	// opt 7 == raw+uniform weight unbinned

	//opt 7 is not available to binned option.
	...
	for (int i=0; i<7; i++){
		...
		obj[i]->gethistograms(opt,use_unbin);
		obj1[i]->gethistograms(opt,use_unbin);
