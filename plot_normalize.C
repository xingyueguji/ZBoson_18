...
void plot_normalize(){
	...
	for (int i=0; i<11; i++){
		mass_array_W[i] = (TH1D*) mc_file->Get(Form("mass_array_W_%i",i));
		mass_array_tt[i] = (TH1D*) mc_file->Get(Form("mass_array_tt_%i",i));
		mass_array_tau[i] = (TH1D*) mc_file->Get(Form("mass_array_tau_%i",i));
		mass_array_signal[i] = (TH1D*) mc_file->Get(Form("mass_array_signal_%i",i));
		mc_A[i] = (TH1D*) mc_file->Get(Form("mc_reco_A_%i",i));
		mass_array_data[i] = (TH1D*) data_file->Get(Form("mass_array_data_%i",i));
		mass_array_samesign[i] = (TH1D*) mc_file->Get(Form("mc_estimate_ss_%i",i));
		data_A[i] = (TH1D*) data_file->Get(Form("data_A_%i",i));
	}
	...
	h_DY_weight = (TH1D*) mc_file->Get("signal_event_weight");
	h_tt_weight = (TH1D*) mc_file->Get("tt_event_weight");
	h_W_weight = (TH1D*) mc_file->Get("W_event_weight");
	...
	for (int i=0; i<11; i++){
		ovo->areanormalize(mc_A[i]);
		ovo->areanormalize(data_A[i]);
		...
		ovo->compositeplot(mass_array_data[i],mass_array_signal[i],mass_array_samesign[i],mass_array_tau[i],mass_array_W[i],mass_array_tt[i],i);
		ovo->acoplot(mc_A[i],data_A[i],i);
		ovo->savehistogram(h_normalized_mc[i],h_normalized_data[i],h_normalized_mc_bk[i],i,data_file);
	}
